...
3. Install required packages (python3, sqlite3, vim, LVM tools, etc.)
...
```bash
sudo ./ops/setup.sh --user trainee --password MySecurePass123
```
...
```bash
sudo ./ops/setup.sh --user $(whoami) --password yourpassword
```
...
### "sudo: ./ops/setup.sh: command not found"

Make the script executable:
```bash
chmod +x ops/setup.sh
sudo ./ops/setup.sh
```
...
```bash
sudo ln -sf /opt/LPIC-1/bin/lpic1 /usr/local/bin/lpic1
```
