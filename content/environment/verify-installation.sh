...
#
# Usage: sudo /opt/LPIC-1/content/environment/verify-installation.sh [--quiet]
...
verify_core_installation() {
    ...
    if [[ -d "$INSTALL_DIR/core" ]]; then
        print_pass "Core engine installed"
    else
        print_fail "Core engine missing"
    fi

    # Check scenarios
    if [[ -d "$INSTALL_DIR/content/scenarios" ]]; then
    ...
    # Check MOTD script
    if [[ -x "$INSTALL_DIR/content/motd/training-motd.sh" ]]; then
    ...
    # Check Textual app
    if [[ -f "$INSTALL_DIR/apps/tui_textual/app.py" ]]; then
        print_pass "Textual app installed"
    else
        print_warn "Textual app missing"
    fi
}
...
verify_critical_commands() {
    ...
    local commands=(
        "python3:Runtime for Textual UI"
    ...
    if python3 -c "import textual" &>/dev/null; then
        print_pass "textual (Python package)"
    else
        print_warn "textual not installed (TUI will not launch)"
    fi
}
