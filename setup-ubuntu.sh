#!/bin/bash
# LPIC-1 Training Platform - Ubuntu/Debian Setup
#
# Usage: sudo ./setup-ubuntu.sh [username]
#
...
if [[ ! -f /etc/debian_version ]]; then
    log_error "This script is for Debian/Ubuntu systems"
    log_info "For Fedora/RHEL, use: sudo ./setup-fedora.sh"
    exit 1
fi
...
echo -e "${BOLD}${CYAN}"
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║           LPIC-1 Training Platform Setup (Ubuntu)             ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
if [[ -d /etc/update-motd.d ]]; then
    ...
    for f in /etc/update-motd.d/10-help-text /etc/update-motd.d/50-motd-news /etc/update-motd.d/80-livepatch /etc/update-motd.d/91-release-upgrade /etc/update-motd.d/95-hwe-eol; do
